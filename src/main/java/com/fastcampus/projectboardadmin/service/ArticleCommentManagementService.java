...
public class ArticleCommentManagementService {
  ...
  public List<ArticleCommentDto> getArticleComments() {
    URI uri = UriComponentsBuilder
        ...
        .queryParam("size", 10000) // TODO : 백엔드에서 페이징 처리를 하지 않고, 전체 게시글을 가져와서 프론트엔드에 페이징 처리를 위임하기 위해 충분히 큰 사이즈를 전달하는 방식으로 처리하였음.불완전함.
        .build()
        .toUri();
  ...
  public ArticleCommentDto getArticleComment(Long articleCommentId) {
    URI uri = UriComponentsBuilder
        .fromHttpUrl(projectProperties.board().url() + "/api/articleComments/" + articleCommentId)
